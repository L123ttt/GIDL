...
def DGI_reconstruction(y, patterns, num_patterns, img_W, img_H, g_factor=0.5):
    ...
    patterns = patterns.view(num_patterns, img_W * img_H)
    # Calculate comput1
...
class Net(nn.Module):
    ...
    def forward(self, x, batch_size, num_patterns, img_W, img_H):
        # 在通道维度上扩展为 num_patterns 个通道
        x = x.expand(-1, num_patterns, -1, -1)
        # 逐元素相乘
        multiplied = self.patterns * x
        ...
        return DGI_R, self.patterns, conv10
